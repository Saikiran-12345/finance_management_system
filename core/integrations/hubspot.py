...
class HubSpotIntegration:
    """
    Complete integration client for HubSpot API.
    ...
    def _get_base_url(self) -> str:
        if self.environment == 'production':
            return f"https://api.hubspot.com/v1"
        return f"https://sandbox.api.hubspot.com/v1"
        
    def _get_headers(self) -> Dict[str, str]:
        return {
            'Authorization': f"Bearer {self.api_key}",
            'Content-Type': 'application/json',
            'X-HubSpot-Version': '2024-01-01',
            'User-Agent': 'FinanceApp/HubSpotClient/1.0'
        }
    ...
    def _handle_response(self, response: requests.Response) -> Dict[str, Any]:
        try:
            ...
            raise IntegrationError(f"HubSpot Request Failed: {e.response.status_code}", payload=e.response.text)
            
    # --- Resource: Users / Customers ---
    
    def create_customer(self, email: str, name: str, metadata: dict = None) -> Dict[str, Any]:
        """Create a new customer profile in HubSpot"""
        payload = {
            'email': email,
            'name': name,
    ...
    def create_transaction(self, amount: int, currency: str, source: str, description: str = "") -> Dict[str, Any]:
        """Process a transaction through HubSpot"""
        payload = {
            'amount': amount,
    ...
    def process_webhook(self, event_type: str, data: dict):
        """Process incoming HubSpot webhooks"""
        logger.info(f"Processing HubSpot webhook: {event_type}")
    ...
    def generate_report(self, start_date: str, end_date: str) -> Dict[str, Any]:
        """Request a comprehensive financial report from HubSpot"""
        payload = {
