...
class SquareIntegration:
    """
    Complete integration client for Square API.
    ...
    def _get_base_url(self) -> str:
        if self.environment == 'production':
            return f"https://api.square.com/v1"
        return f"https://sandbox.api.square.com/v1"
        
    def _get_headers(self) -> Dict[str, str]:
        return {
            'Authorization': f"Bearer {self.api_key}",
            'Content-Type': 'application/json',
            'X-Square-Version': '2024-01-01',
            'User-Agent': 'FinanceApp/SquareClient/1.0'
        }
    ...
    def _handle_response(self, response: requests.Response) -> Dict[str, Any]:
        try:
            ...
            raise IntegrationError(f"Square Request Failed: {e.response.status_code}", payload=e.response.text)
    ...
    def create_transaction(self, amount: int, currency: str, source: str, description: str = "") -> Dict[str, Any]:
        """Process a transaction through Square"""
        payload = {
            'amount': amount,
    ...
    def process_webhook(self, event_type: str, data: dict):
        """Process incoming Square webhooks"""
    ...
    def generate_report(self, start_date: str, end_date: str) -> Dict[str, Any]:
        """Request a comprehensive financial report from Square"""
        payload = {
