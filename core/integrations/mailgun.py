...
class MailgunIntegration:
    """
    Complete integration client for Mailgun API.
    ...
    def _get_base_url(self) -> str:
        if self.environment == 'production':
            return f"https://api.mailgun.com/v1"
        return f"https://sandbox.api.mailgun.com/v1"
        
    def _get_headers(self) -> Dict[str, str]:
        return {
            'Authorization': f"Bearer {self.api_key}",
            'Content-Type': 'application/json',
            'X-Mailgun-Version': '2024-01-01',
            'User-Agent': 'FinanceApp/MailgunClient/1.0'
        }
    ...
    def _handle_response(self, response: requests.Response) -> Dict[str, Any]:
        try:
            ...
            logger.error(f"Mailgun API Error: {e.response.text}")
            raise IntegrationError(f"Mailgun Request Failed: {e.response.status_code}", payload=e.response.text)
            
    # --- Resource: Users / Customers ---
    
    def create_customer(self, email: str, name: str, metadata: dict = None) -> Dict[str, Any]:
        """Create a new customer profile in Mailgun"""
        payload = {
            'email': email,
            'name': name,
    ...
    def create_transaction(self, amount: int, currency: str, source: str, description: str = "") -> Dict[str, Any]:
        """Process a transaction through Mailgun"""
        payload = {
            'amount': amount,
    ...
    def process_webhook(self, event_type: str, data: dict):
        """Process incoming Mailgun webhooks"""
        logger.info(f"Processing Mailgun webhook: {event_type}")
        if event_type == 'transaction.created':
            ...
            logger.warning(f"Unhandled Mailgun webhook event: {event_type}")
    ...
    def generate_report(self, start_date: str, end_date: str) -> Dict[str, Any]:
        """Request a comprehensive financial report from Mailgun"""
        payload = {
